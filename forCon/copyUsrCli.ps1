...
function getNewUserInstance {
    Param([string] $LoginName, $Count)
    $Count = $Count+1
    if ($count -lt 5) {
        try {
            return Get-ADUser -Identity $LoginName
        }
        catch {
            Write-Output "New user was not created or found try again 15 seconds"
            
            Start-Sleep -s 1.5
            getNewUserInstance -LoginName $newSamAccountName -Count $Count
        }
    }else{
        Write-Output "Attempt to create a new user appears to have failed"
        break;
    }
}
